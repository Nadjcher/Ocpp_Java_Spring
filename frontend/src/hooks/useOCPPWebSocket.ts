...
/** Payload OCPP générique - toutes les actions possibles */
export type OCPPPayload = Record<string, unknown>;

/** Réponse BootNotification */
...
export const useOCPPWebSocket = (options: UseOCPPWebSocketOptions) => {
    ...
    const meterValuesInterval = useRef<ReturnType<typeof setInterval> | null>(null);
    ...
    // Send OCPP message
    const sendOCPPMessage = useCallback(<T = OCPPPayload>(action: string, payload: OCPPPayload = {}): Promise<T> => {
    ...
    const handleIncomingCall = useCallback((messageId: string, action: string, _payload: OCPPPayload) => {
        addLog(`← ${action}`);
    ...
    const handleCallResult = useCallback((messageId: string, payload: OCPPPayload) => {
    ...
    // Send CALLRESULT
    const sendCallResult = useCallback((messageId: string, payload: OCPPPayload) => {
        if (ws.current && ws.current.readyState === WebSocket.OPEN) {
    ...
    // OCPP Actions
    const sendBootNotification = useCallback(async (): Promise<BootNotificationResponse> => {
        try {
            const response = await sendOCPPMessage<BootNotificationResponse>('BootNotification', {
    ...
    const authorize = useCallback(async (idTag: string): Promise<AuthorizeResponse> => {
        try {
            const response = await sendOCPPMessage<AuthorizeResponse>('Authorize', { idTag });
    ...
    const startTransaction = useCallback(async (): Promise<StartTransactionResponse> => {
        if (!session.idTag) {
            throw new Error('No idTag available');
        }

        try {
            const response = await sendOCPPMessage<StartTransactionResponse>('StartTransaction', {
                connectorId: 1,
                idTag: session.idTag,
    ...
    const stopTransaction = useCallback(async (): Promise<StopTransactionResponse> => {
        ...
        try {
            const response = await sendOCPPMessage<StopTransactionResponse>('StopTransaction', {
