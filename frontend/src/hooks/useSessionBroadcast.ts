...
export function useSessionBroadcast(
  ...
  const sessionsTobroadcast = useCallback((sessions: SessionState[]): BroadcastSessionData[] => {
    return sessions.map(s => ({
      ...
      cpId: s.cpId,
      status: s.status,
      isConnected: s.status !== 'disconnected',
      isCharging: s.isCharging,
      soc: s.socCurrent,
      offeredPower: s.metrics.txdpKw ? s.metrics.txdpKw * 1000 : 0,
      activePower: s.metrics.txpKw ? s.metrics.txpKw * 1000 : 0,
      setPoint: s.metrics.backendKwMax ? s.metrics.backendKwMax * 1000 : 0,
      energy: s.energyTotal * 1000,
      voltage: s.metrics.voltage || 230,
      current: s.powerCurrent * 1000 / (s.metrics.voltage || 230),
