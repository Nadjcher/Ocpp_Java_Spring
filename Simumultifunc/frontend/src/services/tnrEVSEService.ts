...
export interface ReplayOptions {
    /** Multiplicateur de vitesse: 1 = temps réel, 2 = 2x plus vite, 0.5 = 2x plus lent */
    speedMultiplier?: number;
    /** Si true, ignore les délais et exécute le plus vite possible */
    fastForward?: boolean;
    /** Délai minimum entre événements en ms (pour éviter de surcharger) */
    minDelayMs?: number;
    /** Délai maximum entre événements en ms (0 = pas de limite) */
    maxDelayMs?: number;
}
...
class TNREVSEService extends EventEmitter {
    ...
    private replayAborted: boolean = false;

    private sessionWebSockets: Map<string, WebSocket> = new Map();
    private messageListeners: Map<string, Function> = new Map();

    // Options de replay par défaut: temps réel
    private defaultReplayOptions: ReplayOptions = {
        speedMultiplier: 1,  // Temps réel par défaut
        fastForward: false,
        minDelayMs: 10,      // Minimum 10ms entre événements
        maxDelayMs: 0        // Pas de limite max
    };
    ...
    async replayScenario(recordingId: string, options?: ReplayOptions): Promise<TNRComparison> {
        ...
        const totalDuration = recording.endTime
            ? recording.endTime - recording.startTime
            : 0;
        const estimatedReplayTime = replayOpts.fastForward
            ? recording.events.length * (replayOpts.minDelayMs || 10)
            : totalDuration / (replayOpts.speedMultiplier || 1);

        this.emit('replayStarted', {
            recordingId,
            options: replayOpts,
            estimatedDurationMs: estimatedReplayTime,
        ...
        let lastEventTime = recording.startTime;

        // Rejouer les événements avec le timing correct
        for (let i = 0; i < recording.events.length; i++) {
            // Vérifier si le replay a été annulé
            if (this.replayAborted) {
                this.emit('replayAborted', { recordingId, eventIndex: i });
                break;
            }

            const event = recording.events[i];

            // Calculer le délai depuis le dernier événement
            const timeSinceLastEvent = event.timestamp - lastEventTime;
            lastEventTime = event.timestamp;

            // Appliquer le délai avec les options de vitesse
            let delayMs: number;
            if (replayOpts.fastForward) {
                // Mode fast-forward: délai minimum uniquement
                delayMs = replayOpts.minDelayMs || 10;
            } else {
                // Mode normal: respecter le timing avec multiplicateur
                delayMs = timeSinceLastEvent / (replayOpts.speedMultiplier || 1);

                // Appliquer les limites min/max
                if (replayOpts.minDelayMs && delayMs < replayOpts.minDelayMs) {
                    delayMs = replayOpts.minDelayMs;
                }
                if (replayOpts.maxDelayMs && replayOpts.maxDelayMs > 0 && delayMs > replayOpts.maxDelayMs) {
                    delayMs = replayOpts.maxDelayMs;
                }
            }
            ...
            // Émettre la progression avec plus de détails
            this.emit('replayProgress', {
                current: i + 1,
                total: recording.events.length,
                percentage: Math.round(((i + 1) / recording.events.length) * 100),
                eventType: event.type,
                eventAction: event.action,
                elapsedMs: event.timestamp - recording.startTime,
                delayApplied: delayMs
            });
        }

        // Attendre pour capturer les derniers messages (proportionnel à la vitesse)
        const finalWait = replayOpts.fastForward ? 500 : 2000 / (replayOpts.speedMultiplier || 1);
    ...
    // Définir les options de replay par défaut
    setDefaultReplayOptions(options: ReplayOptions): void {
        this.defaultReplayOptions = {
            ...this.defaultReplayOptions,
            ...options
        };
    ...
    // Obtenir les options de replay par défaut
    getDefaultReplayOptions(): ReplayOptions {
        return { ...this.defaultReplayOptions };
    }
