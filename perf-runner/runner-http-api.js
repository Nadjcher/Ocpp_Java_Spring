...
if (process.env.NODE_ENV === 'production') {
    ...
    if (fsSync.existsSync(frontendPath)) {
        ...
        app.use(express.static(frontendPath, {
            maxAge: '1y',  // Cache les assets pour performance
            etag: false,
            index: false    // On gère index.html manuellement
        }));

        // Route catch-all pour React Router
        // IMPORTANT: Doit être APRÈS toutes vos routes API
        app.get('*', (req, res) => {
        ...
        log('✅ Frontend served from:', frontendPath);
        log('🌐 Application complète sur http://localhost:' + PORT);
        log('========================================');
    } else {
        log('⚠️  Mode production mais frontend/dist n\'existe pas !');
        log('   Lancez d\'abord : cd frontend && npm run build');
    }
} else {
    log('========================================');
    log('  🔧 MODE DÉVELOPPEMENT');
    log('========================================');
    log('Frontend dev : http://localhost:3002');
    log('Backend API  : http://localhost:' + PORT);
    log('========================================');
}
